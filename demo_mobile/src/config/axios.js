...
const accessToken =  __ENV__ === 'development' ? '' : '' //debug
...
function getToken(){
    return new Promise((resolve) => {
        if(__ENV__ !== 'development'){ //非本地开发环境
            window.YYEsnBridge.ready(() => {
                window.YYEsnBridge.do('getToken', {
                    success: (res) => {
                        let resData = {}
                        try{
...
async function getConfig(config){
    ...
    if(!config.params.access_token){
        try{
            config.params.access_token = await getToken()
        } catch(e){
            //
        }
    }
    if(!config.params.lang){
        config.params.lang = Browser.getLanguage()
    }
    return new Promise((resolve) => {
        resolve(config)
    })
